...
class bmicalculator():
     ...
     def __init__(self,master):
          ...
          c.execute('CREATE TABLE IF NOT EXISTS BMIStorageNew(name TEXT,timestamp TEXT,bodymassindex REAL,weightclass TEXT)') 
          ...
          self.label3=Label(self.master,text='Please enter your height in centimetres',fg='black').grid(row=4,column=0)
          self.label4=Label(self.master,text='Please enter your weight in kilograms',fg='black').grid(row=5,column=0)


          self.myName=Entry(self.master,textvariable=self.nametxt).grid(row=3,column=1)
          self.myheight=Entry(self.master,textvariable=self.heightcm).grid(row=4,column=1)
          self.myweight=Entry(self.master,textvariable=self.weightkg).grid(row=5,column=1)
     ...
     def dynamic_data_entry(self):
        ...
        #this is what adds the data to the database. Bmi has to be changed to the value of bmi and weightclass has to be change to the weightclass
          ...
          Name = str(self.nametxt.get())
          timestamp = str(datetime.datetime.now().date())
          bodymassindex = self.bmi
          weightclass = self.totalindex
          c.execute("INSERT INTO BMIStorageNew (Name, timestamp, bodymassindex, weightclass) VALUES (?, ?, ?, ?)",(Name, timestamp, bodymassindex, weightclass))
          conn.commit()
...
class records():
    ...
    def __init__(self,master):
        ...
        self.NameLabel = Label(self.master, text="Name", width=15)
    ...
    def readfromdatabase(self):
        self.cur.execute("SELECT * FROM BMIStorageNew")
